    ...
    <div class="admin-panel-info__new-users admin-panel-info-new-users admin-panel-info-users">
        <div class="admin-panel-info-new-users__container admin-crud">
            <div class="add-course__form add-course-form">
                <form wire:submit.prevent="update({{ $lesson_id }})" enctype="multipart/form-data">
                    <select wire:ignore wire:model="course_id" name="course_id" class="add-course-form__input add-course-form__select">
                        <option value="0" selected disabled>Курс</option>
                        @foreach ($courses as $course)
                        <option @if ($lesson->course_id === $course->id) selected @endif value="{{ $course['id'] }}">{{ $course['name'] }}
                        </option>
                        @endforeach
                    </select>
                    ...
                    {{ $message }}
                    @enderror
                    <textarea wire:ignore wire:model="task" placeholder="Введите задачу..." class="add-course-form__input add-course-form__textarea">{{ $task }}</textarea>
                    @error('task')
                    {{ $message }}
                    @enderror
                    <input wire:ignore value="{{ $lesson_number }}" wire:model="lesson_number" type="text" class="add-course-form__input" placeholder="Номер урока">
                    @error('lesson_number')
                    {{ $message }}
                    @enderror
                    <div class="lesson__files">
                        @foreach($files as $file)
                        <div class="lesson-files__item">
                            <img class="drag-img-styles" src="{{ $file->file->file_urls }}" alt="image">
                            <button wire:click.prevent="deleteFile({{ $file }}, {{ $file->id }})" wire:target='deleteFile({{ $file }}, {{ $file->id }})' wire:loading.class="remove" class="delete"></button>
                        </div>
                        @endforeach
                    </div>
                    <div class="add-course-form__img add-course-form-img dragable-img-multi lesson-edit">
                        <input wire:model="file_path" id="dragImg" type="file" name="file_path[]" multiple class="hidden add-course-form-img__img drag-img" draggable="true">
                        <label for="dragImg" class="add-course-form-img-content add-course-form-img-content__button">
                            ...
                            @if($file_path)
                            <div class="livewire-images-preview">
                                @foreach($file_path as $file)
                                <img src="{{ $file->temporaryUrl() }}" alt="Файл или изображение">
                                @endforeach
                            </div>
                            @else
                            <div class="add-course-form-img-content__name">
                                ...
                                <div wire:loading wire:target="file_path" class="loading add-course-form-img-content-name__text">Загрузка...</div>
                                <p class="add-course-form-img-content-name__text" wire:target="file_path" wire:loading.class="disabled">Перетащите сюда файлы</p>
                            </div>
                            @endif
                        </label>
                        @error('file_path')
                    ...
                    <div class="add-video-column video__inputs">
                        ...
                        <div class="video__input-column">
                            <input wire:model="video.0" type="text" name="inputs[0][video_path]" class="add-course-form__input" placeholder="Введите ссылку на видео">
                            <button wire:click.prevent="add({{$i}})" type="button" class="add-video-paths">Добавить</button>
                        </div>
                    </div>
                    <div class="add-video-column video__inputs">
                        ...
                        <div class="video__input-column">
                            <div class="video__input-column">
                                <input wire:model="video.{{ $value }}" type="text" class="add-course-form__input" placeholder="Введите ссылку на видео">
                                <button wire:click.prevent="remove({{$key}})" type="button" class="remove">Удалить</button>
                            </div>
                        </div>
                        @endforeach
                    </div>
                    <div class="add-video-column video__inputs">
                        ...
                        <div class="video__input-column">
                            <input type="text" class="add-course-form__input" placeholder="Введите ссылку на видео" value="{{ $video->video_path }}">
                            <button wire:click.prevent="deleteVideo({{ $video }})" type="button" class="remove">Удалить</button>
                        </div>
                        @endforeach
                    </div>
                    <div class="add-course-column-button">
                        <a href="{{ url()->previous() }}" class="admin-panel-info-new-users__button">Назад</a>

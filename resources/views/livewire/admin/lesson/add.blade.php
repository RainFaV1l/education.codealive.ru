            ...
            <div class="admin-panel-content__content">
                <div class="admin-panel-content-aside__header admin-panel-content-aside-header">
                    <div class="admin-panel-content-aside-header__container">
                        <h1 class="admin-panel-content-aside-header__title">Добавление урока</h1>
                        <p class="admin-panel-content-aside-header__subtitle">Страница добавления урока</p>
                            ...
                            <div class="add-course__form add-course-form">
                                <form action="{{ route('lessons.store') }}" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="form__item">
                                        <div class="error__input-column">
                                            <div class="input-column @error('name') error @enderror">
                                                <input value="{{ old('lesson_number') }}" type="text" name="lesson_number" class="input add-course-form__input" required autofocus>
                                                <label for="profile-surname">Номер урока</label>
                                            </div>
                                            @error('lesson_number')
                                            <div class="input-column-error__text"> {{ $message }}</div>
                                            @enderror
                                    ...
                                    <textarea name="description" placeholder="Введите описание..." class="add-course-form__input add-course-form__textarea">{{ old('description') }}</textarea>
                                    @error('description')
                                    {{ $message }}
                                    @enderror
                                    <textarea name="task" placeholder="Введите задание..." class="add-course-form__input add-course-form__textarea">{{ old('description') }}</textarea>
                                    @error('task')
                                    {{ $message }}
                                    @enderror
                                    <div class="add-course-form__img add-course-form-img dragable-img-multi">
                                        <input id="dragImg" type="file" name="file_path[]" multiple class="hidden add-course-form-img__img drag-img" draggable="true">
                                    ...
                                    <div class="add-course-column-button">
                                        <a href="{{ route('dashboard.lessons') }}" class="admin-panel-info-new-users__button">Назад</a>
